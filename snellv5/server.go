...
func NewService(options ServiceOptions) (*Service, error) {
	...
	case snell.ObfsModeTLS:
		// snell-server v5.0.1 config only enables HTTP obfs; this package intentionally
		// keeps TLS obfs as a Surge-compatible service extension.
	default:
...
func (s *Service) writePong(conn net.Conn) error {
	// snell-server v5.0.1: FUN_0013f630 handles CommandPing by writing ReplyPong
	// through FUN_0013def0.
	salt := make([]byte, snell.SaltLen)
	_, err := io.ReadFull(rand.Reader, salt)
	if err != nil {
		return err
